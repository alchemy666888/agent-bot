...
Current approval note: The previous approval was invalidated by the reopened persistence requirements. These PostgreSQL tasks must not be implemented; this file will be rewritten after the replacement `design.md` is approved.

Approval note: The prior approval attempt was not accepted because `design.md` was reopened to correct the Vercel region configuration mechanism. This task draft now follows the corrected design and requires a new review after that design is re-approved.
...
- `vercel.json` or route configuration — assign Node.js and the 300-second duration without fixing a Function region in source.
...
- `vercel.json` — Node.js function duration with no fixed Function region.
...
3. Configure 300-second Node.js webhook execution and document selection of the nearest supported Asian region through Vercel project settings or the deployment CLI.
